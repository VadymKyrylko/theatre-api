...
from theatre.models import TheatreHall, Play, Performance, Reservation, Ticket, Genre, Actor
...
class TheatreViewTests(APITestCase):
    def setUp(self):
        ...
        self.staff = User.objects.create_superuser(email="admin@example.com", password="adminpass123")
        self.hall = TheatreHall.objects.create(name="main", rows=4, seats_in_row=5)
        ...
        self.performance = Performance.objects.create(play=self.play, theatre_hall=self.hall,
    ...
    def authenticate(self, user=None, password=None):
        ...
        res = self.client.post(self.token_url, {"email": user.email, "password": password})
    ...
    def test_user_can_create_reservation(self):
        ...
        res = self.client.post(self.reservation_url, data=payload, format="json")
    ...
    def test_user_cannot_reserve_past_performance(self):
        past_performance = Performance.objects.create(play=self.play, theatre_hall=self.hall, show_time=now() - timedelta(days=1))
        self.authenticate()
        payload = {"tickets": [
